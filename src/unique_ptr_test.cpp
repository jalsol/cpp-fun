...
TEST_CASE("unique_ptr<T[]>: destructor releases memory",
          "[unique_ptr][array]") {
  TestObject::reset_counters();
...
TEST_CASE("unique_ptr<T[]>: release() transfers ownership",
          "[unique_ptr][array]") {
  TestObject::reset_counters();
...
TEST_CASE("unique_ptr<T[]>: copy constructor is deleted",
          "[unique_ptr][array]") {
  STATIC_REQUIRE_FALSE(std::is_copy_constructible_v<jal::unique_ptr<int[]>>);
}

TEST_CASE("unique_ptr<T[]>: copy assignment is deleted",
          "[unique_ptr][array]") {
  STATIC_REQUIRE_FALSE(std::is_copy_assignable_v<jal::unique_ptr<int[]>>);
}

TEST_CASE("unique_ptr<T[]>: move semantics in container",
          "[unique_ptr][array]") {
  TestObject::reset_counters();
  ...
  vec.push_back(
    jal::unique_ptr<TestObject[]>(new TestObject[3]{{10}, {20}, {30}}));
